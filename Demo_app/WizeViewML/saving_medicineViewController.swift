//
//  saving_medicineViewController.swift
//  Demo_app
//
...
class saving_medicineViewController: UIViewController{
        ...
        transferManager.upload(request).continueWith(executor: AWSExecutor.mainThread()) { (task) -> Any? in
            ...
            if task.result != nil {
                ...
                AWSS3PreSignedURLBuilder.default().getPreSignedURL(getPreSignedURLRequest).continueWith { (task:AWSTask<NSURL>) -> Any? in
                    ...
                    var request = URLRequest(url: URL(string: "https://api.wizeview.com/v1/medicines/\(self.medicineid)/pictures")!)
                        ...
                        if self.statuscode==200 || self.statuscode == 201 {
                            ...
                            if let responseJSON = responseJSON as? [String:Any]{
                                DispatchQueue.main.async {
                                    ...
                                    self.result.append(responseJSON["name"] as! String)
                                    self.result.append(responseJSON["strength"] as! String)
                                    self.result.append(responseJSON["form"] as! String)
                                    self.result.append(responseJSON["quantity"] as! String)
                                    self.result.append(responseJSON["fillDate"] as! String)
                                    self.result.append(responseJSON["expireDate"] as! String)
                                    //self.result.append(responseJSON["refills"] as! String)
                                    //self.result.append(responseJSON["dosage"] as! String)
                                    //self.result.append(responseJSON["quantity_on_hand"] as! String)
    ...
    func resulttable() {
        ...
        performSegue(withIdentifier: "m_upload_to_table", sender: self)
        
    }
    
    func errortable() {
        
        performSegue(withIdentifier: "m_upload_to_errortable", sender: self)
    ...
    {
        if segue.destination is medicine_tableViewController
        {
            let vc = segue.destination as? medicine_tableViewController
            vc?.result = result
        }
        
        if segue.destination is medicine_error_tableViewController{
            let vc = segue.destination as? medicine_error_tableViewController
            vc?.error = error
        }
    }
